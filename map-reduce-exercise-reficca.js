// Here are some simple problems to solve using the native .reduce and .map array methods. 
// Each of these problems can be solved in many different ways, but you should solve them using the 
// requested higher order function.
...
const capitalize = function(string){
    return string.split('').map(c => c.toUpperCase()).join('');
...
//   ex: swapCase('hey gurl, lets javascript together sometime')
//    => "HEY gurl, LETS javascript TOGETHER sometime"
  ...
  const swapCase = function(string){
    ...
    let mapOutput = wordsArr.map((word, index) => {
      if (index % 2 === 0) {
  ...
  const shiftLetters = function(string){
    let mapOutput = string.split("").map((letter) => {
      let newCharCode = letter.charCodeAt(0)+1;
...
const decode = function(string){
  let decodedString = string.split(" ").map(codeBlock => {
    let finalCode = codeBlock.split("").reduce((acc, curVal) => {
      acc = acc + parseInt(curVal);
      return acc;
    }, 0);
